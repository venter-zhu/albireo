...
public class JdbcDataProvider extends DataProvider implements Aggregatable, Initializing {
        ...
        try (Connection con = getConnection();
            ...
            while (rs.next()) {
                ...
                if (resultCount > resultLimit) {
                    throw new AlbireoException("Cube result count " + resultCount + ", is greater than limit " + resultLimit);
                }
            }
            executor.shutdown();
    ...
    private Connection getConnection() throws Exception {
        ...
        if (usePool != null && "true".equals(usePool)) {
            String key = Hashing.sha256().newHasher().putString(JSONObject.toJSON(dataSource).toString(), Charsets.UTF_8).hash().toString();
